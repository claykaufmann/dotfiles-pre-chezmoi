...
Set var and fixed pitch fonts, this is mainly for Org mode:
#+begin_src emacs-lisp
(custom-set-faces!
...
Turned this off for now as I overall prefer it, the issue is if I double return, it goes to 2 indents down, which is really funky.
#+begin_src emacs-lisp
...
** Visuals
[[https://zzamboni.org/post/beautifying-org-mode-in-emacs/][This article]] has some great tips for customizing org mode to look much better. The following few code blocks take some stuff from that article to make org mode look much better. I also took stuff from [[https://lepisma.xyz/2017/10/28/ricing-org-mode/][ricing up Org mode]].
...
*** Non-prose version
The following code block is for using non-prose org mode. All it does is slightly modify headlines and the doc title.
#+begin_src emacs-lisp
...
*** Prose Version
The next few codeblocks make org mode look like a normal word processor, and in general much fancier. It is an alternative to the above.
...
Enable variable pitch fonts in org, and a couple other hooks for cleaning up the UI:
#+begin_src emacs-lisp
...
Disable ~hl-line-mode~ in org mode:
(if any hl-line-mode issues arise, check here!)
#+begin_src emacs-lisp
...
Set the cursor to always be a bar:
Disabled for now as it was annoying...
#+begin_src emacs-lisp
...
The following code blocks make specific org fonts look different.

Modify headings to look better:
#+begin_src emacs-lisp
(custom-set-faces!
...
Modify doc title and meta keywords:
#+begin_src emacs-lisp
(custom-set-faces!
...
Modify org-verbatim to highlight:
#+begin_src emacs-lisp
